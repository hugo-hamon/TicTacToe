...
from tictactoe import TicTacToe, Player, get_opponent
...
def alphabeta(game: TicTacToe, depth: int, alpha: float, beta: float, maximizing: bool, player: Player) -> tuple[float, Optional[tuple[int, int]]]:
...
def get_best_move(game: TicTacToe, depth: int, player: Player, algorithm: Algorithm) -> tuple[int, int]:
    ...
    if algorithm == Algorithm.ALPHABETA:
        score, move = alphabeta(
            game, depth, float('-inf'), float('inf'), True, player
        )
        print(f"Score: {score}")
